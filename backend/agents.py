...
class AgentConfig:
	...
	llm_model: Optional[str] = None  # e.g., "gpt-4.1" or "gpt-4o-mini"
...
class BaseAgent:
	def __init__(self, name: str, config: Optional[AgentConfig] = None):
		...
		# arbitrary prompt variables you can extend over time
		self.prompt1: str = ""
		# private base prompt buffer (accumulated instructions/goals)
		self._prompt: str = ""
		# conversation memory: list of role/content messages
	...
	def add_message(self, role: str, content: str) -> None:
		# role usually one of: user/assistant/system
	...
	def _history_to_text(self, max_chars: int = 2000) -> str:
		# Join messages into a single text blob, truncated to max_chars
		joined = "\n".join(f"{m.get('role','unknown')}: {m.get('content','')}" for m in self._history)
...
class AgentOrchestrator:
	...
	def sustainability_agent(self):
		...
		if self._sustainability_agent is None:
			from backend.agents.specialized_agents import SustainabilityAgent
	...
	def indigenous_context_agent(self):
		...
		if self._indigenous_context_agent is None:
			from backend.agents.specialized_agents import IndigenousContextAgent
